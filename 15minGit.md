...
Git作为一个复杂的系统，学习起来比较费时间，相信很多小白已经望而却步了。标题起个15分钟是个噱头，这里的教程，不是直接去讲Git命令行，那样子没有那么快的~哈哈

而是借助SourceTree这个图形化软件，降低Git的门槛，带着你直觉化的知道Git到底做了什么，当然也会讲解一些原理和命令。不会彻底脱离Git谈Git。毕竟最终的归宿还是CLI

###谈谈版本控制工具

版本控制工具，现在有名的就是SVN和Git吧，其他的也有
这两个比较主流，而Git就是为了取代SVN而存在的。

小白会好奇，版本控制工具是干嘛的？

版本控制工具，就是一个追踪器，能够记录一个文件的每一个改动，这样子仿佛有一个7x24的备份服务器，一直在后面帮你备份着数据，仿佛你有了一个时光机，可以回到文件的任何时候，每当你想查看了，或者回退，你可以轻轻松松搞定。

还有的就是顺带加了合并功能，当一个小团队在共同完成一份作品的不同部分的时候，作品可以是代码，可以是文档，可以是书。版本控制工具，可以合并这些修改，使之变成一个整体。

还有分支……吧啦吧啦，太细就枯燥了

说到这，你是不是还有点小激动呢？

是不是觉得这就是你特别想要的东西？而且细细想想，这个工具似乎每个地方都需要，只要是不断修改的东西。
...
简单的说，SVN就有一个中央服务器，协调大家的代码，大家都和它同步;Git是分布式的，每个人都保留完整的追踪记录，每个人可以成为一个中心。所以说SVN缺点就是，中心宕掉了，所有人都歇菜。Git人人皆可为中心。
